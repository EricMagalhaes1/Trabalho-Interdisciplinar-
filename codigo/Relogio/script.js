...
function updateClock() {
    ...
    document.querySelector('.digital-clock .hours').textContent = padNumber(hours);
    document.querySelector('.digital-clock .minutes').textContent = padNumber(minutes);
    document.querySelector('.digital-clock .seconds').textContent = padNumber(seconds);
    document.querySelector('.digital-clock .day').textContent = padNumber(day);
    document.querySelector('.digital-clock .month').textContent = padNumber(month);
...
function updateTimer() {
    ...
    if (remainingTime < 0) {
        ...
        const newMinutes = Math.floor((remainingTime % (60 * 60 * 1000)) / (60 * 1000));
        const newSeconds = Math.floor((remainingTime % (60 * 1000)) / 1000);
        const newMilliseconds = remainingTime % 1000;
        timerDisplay.textContent = `${padZero(newHours, 2)}:${padZero(newMinutes, 2)}:${padZero(newSeconds, 2)}.${padZero(newMilliseconds, 3)}`;
...
timerStart.addEventListener('click', function() {
    ...
    totalMilliseconds = ((hours * 60 * 60 + minutes * 60 + seconds) * 1000) + milliseconds;
...
timerResume.addEventListener('click', function() {
    if (remainingMilliseconds > 0) { // Ensure there's time remaining
...
lapButton.addEventListener('click', function() {
    if (stopwatchInterval) { // Check if the stopwatch is running
        const now = Date.now();
